  ...
  if (!characterEntity)
  {
   ...
   if(m_inputMap->GetTrigger(PI_TargetDest) > 0.0f)
    {	
		...
		Inventory::InventoryManager().setIsUsingInvertory(false);
		CheckIfInventory();
		
	if( Inventory::InventoryManager().getIsUsingInventory()== false)
	{
...
if (!Vision::CollisionToolkit.TraceLine(traceStart,traceEnd,VIS_TRACE_ALL,VIS_TRACE_ALL,characterEntity,NULL))
{
  // something is picked by mouse cursor
...
{
	Vision::Message.Add(1, " NOT at point");
}*/
...
void PlayerDialog::CheckIfInventory()
{
	if(m_inputMap->GetTrigger(PI_TargetDest) > 0.0f)
    {
		IVGUIContext *const context1 = GetContext();
        VASSERT(context1);
        hkvVec2 const getPos = GetCursorPosition(context1);
		//send pos to inventory
	  Inventory::InventoryManager().CheckTouch(getPos);
	}
}
